...
daegu <- geo %>% group_by(date_report) %>%
  summarize(
    cases=sum(Daegu, na.rm=TRUE)
  )

cases <- filter(daegu, date_report <= as.Date("2020-03-16"), date_report >= as.Date("2020-02-18"))$cases

reconstruct_time_series_daegu_raw <- vector('list', length(ee$b_Intercept))
...
for (i in 1:length(ee$b_Intercept)) {
  ...
  reconstruct_time_series_daegu_raw[[i]] <- sapply(1:length(cases), function(x) {
    data_frame(
      confirm=as.Date("2020-02-18")+x-1,
...
save("reconstruct_time_series_daegu_raw", file="reconstruct_time_series_daegu_raw.rda")
