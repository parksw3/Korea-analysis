library(dplyr)
load("reconstruct_time_series_seoul.rda")
load("degree_censor.rda")

reconstruct_censor <- R_t_seoul_censor <- vector('list', length(reconstruct_time_series_seoul))

set.seed(101)
for (i in 1:length(reconstruct_time_series_seoul)) {
  rr <- reconstruct_time_series_seoul[[i]]
...
save("R_t_seoul_censor", "reconstruct_censor", file="R_t_seoul_censor.rda")
