library(dplyr)
load("reconstruct_time_series_gyeonggi.rda")
load("degree_censor.rda")

reconstruct_censor <- R_t_gyeonggi_censor <- vector('list', length(reconstruct_time_series_gyeonggi))

set.seed(101)
for (i in 1:length(reconstruct_time_series_gyeonggi)) {
  rr <- reconstruct_time_series_gyeonggi[[i]]
  ...
  R_t_gyeonggi_censor[[i]] <- data.frame(
...
save("R_t_gyeonggi_censor", "reconstruct_censor", file="R_t_gyeonggi_censor.rda")
