...
cases <- filter(seoul, date_report <= as.Date("2020-03-16"))$cases3

reconstruct_time_series_seoul <- vector('list', length(ee$b_Intercept))
...
for (i in 1:length(ee$b_Intercept)) {
  ...
  reconstruct_time_series_seoul[[i]] <- sapply(1:length(cases), function(x) {
    data_frame(
...
save("reconstruct_time_series_seoul", file="reconstruct_time_series_seoul.rda")
