...
daegu <- geo %>% group_by(date_report) %>%
  ...
  merge(test_weight) %>%
  mutate(
    cases2=round(cases*weight)
  )

cases <- filter(daegu, date_report <= as.Date("2020-03-16"))$cases2

reconstruct_time_series_daegu_detect <- vector('list', length(ee$b_Intercept))
...
for (i in 1:length(ee$b_Intercept)) {
  ...
  reconstruct_time_series_daegu_detect[[i]] <- sapply(1:length(cases), function(x) {
    data_frame(
...
save("reconstruct_time_series_daegu_detect", file="reconstruct_time_series_daegu_detect.rda")
