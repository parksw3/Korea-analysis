...
\jd{Need to discuss how to present this; we don't want it to get in the way.}
...
\jd{This looks like backward R, which is fine, but not what we said we would do.}
For each posterior sample of the reconstructed incidence time series, we divided daily incidence by the probability that they would have been reported before March 16, 2020, \jd{Can't we make this correction part of the reconstruction step?} and estimated the time-dependent reproduction number using the renewal equation with a 14-day sliding window \citep{fraser2007estimating}:
\begin{equation}
...
\jd{Is this in fact good? Would smoothing be better?}
