...
\section*{Abstract - 75 word limit; currently 67.}
...
Unlike the epidemic in Daegu, which is characterized by a single, large peak followed by a gradual decrease, the epidemic in Seoul consists of several small outbreaks (\fref{data}; see Supplementary Materials for details).
...
To estimate the time-dependent reproduction number $R_t$ (the average number of secondary cases caused by an individual infected at time $t$ \citep{fraser2007estimating}), we first estimated daily incidence of infection from the daily number of reported cases by the KCDC \citep{kcdc}.
...
Then, we inferred onset-to-confirmation delay distributions from the partial line list;
we combined the inferred delay distribution with previously estimated incubation period distribution (Table 1) to obtain probability distributions for date of infection for each reported case.
...
In Daegu, incidence peaked shortly after the first case was confirmed and decreased (\fref{eff}A).
...
In Seoul, incidence decreases slightly as traffic decreased (\fref{eff}B) but estimates of $\mathcal R_t$ remain around 1 throughout (\fref{eff}D).
...
in particular, the increase in the number of tests as well as the improvement in the accuracy of case identification (Supplementary Materials).
...
We also did not distinguish local and imported cases, which may overestimate $\mathcal R_t$ \citep{thompson2019improved}.
We performed a separate analysis of the epidemic in Seoul that accounts for imported cases using line list provided by the Seoul Metropolitan Government; our qualitative conclusions remained robust (Supplementary Materials).
