...
cases <- filter(daegu, date_report <= as.Date("2020-03-16"))$cases3

reconstruct_time_series_daegu <- vector('list', length(ee$b_Intercept))
...
for (i in 1:length(ee$b_Intercept)) {
  ...
  reconstruct_time_series_daegu[[i]] <- sapply(1:length(cases), function(x) {
    data_frame(
...
save("reconstruct_time_series_daegu", file="reconstruct_time_series_daegu.rda")
