library(dplyr)
load("../analysis_reconstruct/reconstruct_time_series_daegu_raw.rda")
load("../analysis_delay/degree_censor.rda")

reconstruct_censor_raw <- R_t_daegu_censor_raw <- vector('list', length(reconstruct_time_series_daegu_raw))

set.seed(101)
for (i in 1:length(reconstruct_time_series_daegu_raw)) {
  rr <- reconstruct_time_series_daegu_raw[[i]]
  ...
  R_t_daegu_censor_raw[[i]] <- data.frame(
...
save("R_t_daegu_censor_raw", "reconstruct_censor_raw", file="R_t_daegu_censor_raw.rda")
