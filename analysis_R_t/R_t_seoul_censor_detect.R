library(dplyr)
load("../analysis_reconstruct/reconstruct_time_series_seoul_detect.rda")
load("../analysis_delay/degree_censor.rda")

reconstruct_censor_detect <- R_t_seoul_censor_detect <- vector('list', length(reconstruct_time_series_seoul_detect))

set.seed(101)
for (i in 1:length(reconstruct_time_series_seoul_detect)) {
  rr <- reconstruct_time_series_seoul_detect[[i]]
  ...
  R_t_seoul_censor_detect[[i]] <- data.frame(
...
save("R_t_seoul_censor_detect", "reconstruct_censor_detect", file="R_t_seoul_censor_detect.rda")
