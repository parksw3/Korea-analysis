library(dplyr)
load("reconstruct_time_series_gyeongbuk.rda")
load("degree_censor.rda")

reconstruct_censor <- R_t_gyeongbuk_censor <- vector('list', length(reconstruct_time_series_gyeongbuk))

set.seed(101)
for (i in 1:length(reconstruct_time_series_gyeongbuk)) {
  rr <- reconstruct_time_series_gyeongbuk[[i]]
  ...
  R_t_gyeongbuk_censor[[i]] <- data.frame(
...
save("R_t_gyeongbuk_censor", "reconstruct_censor", file="R_t_gyeongbuk_censor.rda")
